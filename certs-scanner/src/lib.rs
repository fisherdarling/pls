...
impl Default for ScanConfig {
    fn default() -> Self {
        ScanConfig::from(&certs_settings::Settings::default())
    }
}
        ...
        while let Ok(entries) = files_rx.recv() {
            for entry in entries {
                ...
                pool.spawn(move || {
                    ...
                    let parser = certs_parser::Parser::new(&map);
...
impl From<&certs_settings::Settings> for ScanConfig {
    fn from(settings: &certs_settings::Settings) -> Self {
        Self {

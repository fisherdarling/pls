...
use clap::Parser;
use pls_scanner::{ScanConfig, ScanResult};
use pls_types::Timestamp;
...
impl ScanArgs {
    pub fn run(self, ctx: &Ctx) -> anyhow::Result<()> {
        ...
        let (files_rx, files_seen_rx) = pls_scanner::scan(ScanConfig::from(&settings));

        let mut files_seen = 0;
        let mut buffer = Vec::new();

        let mut index = pls_cert_index::CertIndex::new();
